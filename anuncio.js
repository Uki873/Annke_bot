            ...
            if (!canal) {
                ...
                message.channel.send(`** | Qual será a mensagem do anúncio?**`).then(msg2 => {
                    ...
                    .on('collect', c => {
                        ...
                        message.channel.send(`**:question::grey_question: | Qual será o titulo do anúncio?**`).then(msg3 => {
                            let ck = message.channel.createMessageCollector(x => x.author.id == message.author.id, {max: 1})
                            .on('collect', c => {
                                title = c.content
 
                                message.channel.send(`**:white_check_mark: | O anúncio foi enviado para o canal ${canal} com sucesso**`)
 
                                let embed = new Discord.MessageEmbed()
 
                                .setColor('GOLD')
                                .setTimestamp()
                                .setFooter(`:dizzy: **|** Este anuncio foi criado por: ${message.author.username}`, message.author.displayAvatarURL({format: "png"}))
                                .setTitle(title)
                                .setDescription(desc)
