...
module.exports = {
 ...
 if (!args[0]) return message.channel.send(`**${message.author.username}** utilize: **B!sorteio <tempo> <#canal> <prêmio>**`);
 if (
 !args[0].endsWith("d") &&
 !args[0].endsWith("h") &&
 !args[0].endsWith("m")
 ...
 if (isNaN(args[0][0])) return message.channel.send(`**${message.author.username}** o:| Isso não é um número!`);
 let channel = message.mentions.channels.first();
 if (!channel)
 return message.channel.send(
 `**${message.author.username}** :x: | Eu não consegui encontrar aquele canal na guilda!`
 );
